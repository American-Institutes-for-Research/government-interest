...
in.cite_1 <-
  as.data.frame(tbl(my_db, "temp_5yr_citations_by_cite_yr1"))
in.cite_2 <-
  as.data.frame(tbl(my_db, "temp_5yr_citations_by_cite_yr2"))
in.cite_3 <-
  as.data.frame(tbl(my_db, "temp_5yr_citations_by_cite_yr3"))
in.cite_4 <-
  as.data.frame(tbl(my_db, "temp_5yr_citations_by_cite_yr4"))
in.cite_5 <-
  as.data.frame(tbl(my_db, "temp_5yr_citations_by_cite_yr5"))

### read in tables from local file
in.sector <-
  read.csv(
    "./data_to_read/assignees_lookedup_types.csv",
    header = TRUE,
    stringsAsFactors = FALSE
  )
in.fund <-
  read.csv(
    paste("data_to_read", "Agencies.csv", sep = .Platform$file.sep),
    header = TRUE,
    stringsAsFactors = FALSE
  )
in.size <-
...
in.patent_level <-
  in.patent_level %>% filter(year != "NULL" &
                               year >= 1980 & year <= 2018) # filter on years
patents.keep_ids <- in.patent_level$patent_id

# filter on gov_level ids to get the right data
in.gov_level <-
  in.gov_level %>% filter(patent_id %in% patents.keep_ids)
# merge data two main data files
in.patent_level.merged <-
  merge(in.patent_level, in.gov_level, by = "patent_id")
...
long.5a <-
  as.data.frame(table(in.patent_level$wipo_field, in.patent_level$year))
keep.5a <- long.5a %>%
  select(wipo_field = Var1,
         year = Var2,
         freq = Freq) %>%
...
g.5l
ggsave (
  filename = paste0("./data_viz/longWipoFields_v", script_v, ".png"),
  plot = g.5l,
  device = "png"
)
#########################################################################################################
...
freq.wipo_field.all <-
  as.data.frame(table(in.all.since_2005$wipo_field))
freq.wipo_field.all <- freq.wipo_field.all %>%
  select(wipo_field = Var1, freq = Freq)


# determine overall GI patent ratio vis-a-vis all patents, by wipo field
freq.wipo_field.merge <-
  merge (freq.wipo_field, freq.wipo_field.all, by = "wipo_field")
freq.wipo_field.merge <-
  freq.wipo_field.merge %>% mutate(ratio = freq.x / freq.y)


long.5b <-
  as.data.frame(table(in.all.since_2005$wipo_field, in.all.since_2005$year))
long.5b <-
  long.5b %>% select(wipo_field = Var1,
                     year = Var2,
                     freq = Freq)
long.5a <-
  long.5a %>% select(wipo_field = Var1,
                     year = Var2,
                     freq = Freq)
merged.long5 <- long.5a %>%
  inner_join(long.5b,
             by = c("wipo_field", "year"),
             suffixes = c("_5a", "_5b")) %>%
...
g.5lp
ggsave (
  filename = paste0("./data_viz/longWipoFieldsPercent_v", script_v, ".png"),
  plot = g.5lp,
  device = "png"
)
#########################################################################################################
...
in.fund$FederalIndex <-
  in.fund$Total.R.D / in.fund[in.fund$Fiscal.Year == 1980, which(colnames(in.fund) ==
                                                                   "Total.R.D")] * 100
g.9 <-
  ggplot(data = in.fund, aes(x = Fiscal.Year, y = Total.R.D, group = 1)) + geom_line(size =
                                                                                       1.5, color = "#0066CC") +
...
freq.all.by_year$freqIndex <-
  freq.all.by_year$freq / freq.all.by_year[freq.all.by_year$year == "1980", which(colnames(freq.all.by_year) ==
                                                                                    "freq")] * 100
...
g.10 <-
  ggplot(data = freq.all.by_year, aes(x = year, y = freq, group = 1)) +
...
freq.gi.by_year.clnd$freqIndex <-
  freq.gi.by_year.clnd$freq / freq.gi.by_year.clnd[freq.gi.by_year.clnd$year ==
                                                     "1980", which(colnames(freq.gi.by_year.clnd) == "freq")] * 100

unique(freq.gi.by_year.clnd$year)
g.11a <-
  ggplot(data = freq.gi.by_year.clnd, aes(x = year, y = freq, group = 1)) +
...
merged.ind1 <-
  freq.gi.by_year.clnd %>% inner_join(freq.all.by_year ,
                                      by = "year",
                                      suffixes = c("_gi", "_all"))
merged.ind2 <-
  merged.ind1 %>% inner_join(in.fund, by = c("year" = "Fiscal.Year"))
...
g.12 <- ggplot(melt.ind) +
  ...
  scale_colour_manual(
    values = c(cyan, darkGrey, darkGreen),
    labels = c(
...
ggsave (
  filename = paste0("./data_viz/indexed_", script_v, ".png"),
  plot = g.12,
  device = "png"
)
#########################################################################################################
...
patent_level.bkp$year <-
  as.numeric(as.character(patent_level.bkp$year))
gi.agg <-
  aggregate(patent_level.bkp[, c(2, 3)],
            list(Year = patent_level.bkp$year),
            na.rm = TRUE,
            mean)
...
all.agg <-
  aggregate(all.bkp[, c(2, 3)], list(Year = all.bkp$year), na.rm = TRUE, mean)

merged.agg <-
  gi.agg %>% inner_join(all.agg, by = "Year", suffixes = c("_gi", "_all"))
melt.agg <-
  melt(merged.agg, id = "Year", measure.vars = c(2, 4)) # was c(2:5)
...
g.13 <- ggplot(melt.agg) +
  ...
  scale_colour_manual(
    values = c(cyan, darkGrey),
    labels = c("Government interest patents", "All patents")
  ) +
  scale_linetype_manual(
...
ggsave (
  filename = paste0("./data_viz/longInventor_v", script_v, ".png"),
  plot = g.13,
  device = "png"
)
#########################################################################################################
...
in.assignees$name <-
  paste (in.assignees$name_first, in.assignees$name_last, sep = " ")
in.assignees$entity <- in.assignees$organization
in.assignees$entity[which(in.assignees$entity == "")] <-
  in.assignees$name[which(in.assignees$entity == "")]
...
assignees.merged <-
  assignees.clnd %>%  inner_join(in.patent_level.merged, by = "patent_id")
assignees.merged$entity<-iconv(assignees.merged$entity,"WINDOWS-1252","UTF-8")
assignees.merged$entity <-
  toTitleCase(tolower(assignees.merged$entity))



# 17. sankey viz for funders --> assignees to do the network viz

assignees.merged.sub <-
  assignees.merged[, c(1, 3, 20)] # patent_id, entity, level_one
...
count.ass_org.srtd <-
  count.ass_org.srtd[!grepl(
    "The United States of America as Represented by the United States Department of Energy",
    count.ass_org.srtd$entity
  ),]
count.ass_org.srtd <-
  count.ass_org.srtd[!grepl(
...
source_lst <- lapply(source, function(x)
  match(x, nodes))
count.ass_org.srtd$source <- unlist(source_lst)
# create target
target <- list(count.ass_org.srtd$entity)
target_lst <- lapply(target, function(x)
  match(x, nodes))
count.ass_org.srtd$target <- unlist(target_lst)

# impute funder node ids onto original counts
funders.links <-
  sapply(1:length(count.ass_org.srtd), function(x)
    grep(nodes[x], count.ass_org.srtd[, 2], ignore.case = TRUE))
fl.df <-
  data.frame(ID = rep(seq(funders.links), sapply(funders.links, length)), Obs = unlist(funders.links))
fl.df[, 1] <- fl.df[, 1] - 1
...
assignees.links <-
  sapply(1:nrow(count.ass_org.srtd), function(x)
    grep(nodes[x], count.ass_org.srtd[, 1], ignore.case = TRUE))
...
assignees.links<-lapply(assignees.links, null_to_na)
al.df <-
  data.frame(ID = rep(seq(assignees.links), sapply(assignees.links, length)),
             Obs = unlist(assignees.links))
...
htmlwidgets::saveWidget(
  p.sk,
  file = paste0(
    "out\\assignees\\Sankey org name_",
    script_v,
    "_",
    ".html"
  )
...
g.18b <-
  ggplot(aes(
    y = count,
    x = reorder(level_one,-count),
    fill = thes_types
  ), data = count.sector_org.short) +
...
ggsave (
  filename = paste0("./data_viz/funders-assignees.dodged_", script_v, ".png"),
  plot = g.18b,
  device = "png"
)
#########################################################################################################
...
patent_size.merged.clnd[which(patent_size.merged.clnd$thes_types == "Academic or Hospital"), 7] <-
  "Academic or Hospital"
patent_size.merged.clnd[which(
  patent_size.merged.clnd$thes_types == "Corporate" &
    patent_size.merged.clnd$size_issue == "micro"
), 7] <- "Small firm"
patent_size.merged.clnd[which(
  patent_size.merged.clnd$thes_types == "Corporate" &
    patent_size.merged.clnd$size_issue == "small"
), 7] <- "Small firm"
patent_size.short <-
  patent_size.merged.clnd %>% select(patent_id, thes_types, year, size_issue) # patent_id, thes_types (sector), and type
patent_size.unique <-
  distinct(patent_size.short) # unique because sector_org contains gi orgs and we don't need that info here
...
g.19n <-
  ...
  scale_colour_manual(
    values = c(darkGreen, darkBlue, cyan),
    labels = c("Academic or hospital", "Large firm", "Small firm")
  ) +
  scale_linetype_manual(
...
ggsave (
  filename = paste0("./data_viz/firmSize_", script_v, ".png"),
  plot = g.19n,
  device = "png"
)
#########################################################################################################
...
in.cite_1_sum <- in.cite_1 %>%
  ...
  summarise(num_citation = sum(num_times_cited_by_us_patents) /
              n()) %>%
...
cit.merge_3 <-
  cit.merge_2 %>% left_join(
    in.cite_3_sum,
    by = c("patent_id" = "patent_id"),
    suffix = c("_yr2", "_yr3")
  )
cit.merge_4 <-
  cit.merge_3 %>% left_join(
    in.cite_4_sum,
    by = c("patent_id" = "patent_id"),
    suffix = c("_yr3", "_yr4")
  )
cit.merge_5 <-
  cit.merge_4 %>% left_join(
    in.cite_5_sum,
    by = c("patent_id" = "patent_id"),
    suffix = c("_yr4", "_yr5")
  )
sector_merge <-
  cit.merge_5 %>% left_join(sector_org.merged.ratio.clnd, by = "patent_id")

#calculate the weight for each year
sector_merge$First <-
  sector_merge$num_citation_yr1 * sector_merge$weight
sector_merge$Second <-
  sector_merge$num_citation_yr2 * sector_merge$weight
sector_merge$Third <-
  sector_merge$num_citation_yr3 * sector_merge$weight
sector_merge$Fourth <-
  sector_merge$num_citation * sector_merge$weight
sector_merge$Fifth <-
  sector_merge$num_citation_yr4 * sector_merge$weight
sector_merge.sub <- sector_merge[, c(23:30)]

sector_merge.sub[is.na(sector_merge.sub)] <- 0
cit_sector.count <-
  aggregate(
    cbind (weight, First, Second, Third, Fourth, Fifth) ~
      thes_types,
    sum,
    na.rm = TRUE,
    data = sector_merge.sub
  )
cit_sector.count.clnd <-
  cit_sector.count %>% filter(thes_types != 0)


# average by year
cit_sector.count.clnd$First <-
  cit_sector.count.clnd$First / cit_sector.count.clnd$weight
cit_sector.count.clnd$Second <-
  cit_sector.count.clnd$Second / cit_sector.count.clnd$weight
cit_sector.count.clnd$Third <-
  cit_sector.count.clnd$Third / cit_sector.count.clnd$weight
cit_sector.count.clnd$Fourth <-
  cit_sector.count.clnd$Fourth / cit_sector.count.clnd$weight
cit_sector.count.clnd$Fifth <-
  cit_sector.count.clnd$Fifth / cit_sector.count.clnd$weight
cit_sector.count.melt <-
  melt(cit_sector.count.clnd,
       id = "thes_types",
       measure.vars = c(3:7))
cit_sector.count.melt.clnd <-
  cit_sector.count.melt %>% filter(thes_types != "Other")

g.27 <-
  ggplot(cit_sector.count.melt.clnd,
         aes(x = variable, y = value, fill = thes_types)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(y = "Average Accrued Weighted Citations", x = "Year After Publication") +
  scale_y_continuous(label = comma) +
  scale_fill_manual(
    values = c(darkPurple, cyan, darkGreen),
    labels = c("Academic", "Corporate", "Government")
  ) +
  theme_set(theme_gray(base_size = 16)) +
  theme(
    legend.direction = 'horizontal',
...
g.27
ggsave (
  filename = paste0("./data_viz/fiveYearCitationImpact_", script_v, ".png"),
  plot = g.27,
  device = "png"
)
#########################################################################################################
# 28. data table alinging sector, agency, and field, by year
sector_org_field.by_year.merged <-
  merge(sector_org.merged.ratio.clnd, in.patent_level, by = "patent_id")
colnames (sector_org_field.by_year.merged)
head(sector_org_field.by_year.merged)
sector_org_field.by_year.small <-
  sector_org_field.by_year.merged[c(1, 2, 5, 10, 11)]
sector_org_field.by_year.ratio <-
  sector_org_field.by_year.small %>%
  group_by(patent_id) %>%
  mutate(weight = 1 / n())

sector_org_field.by_year.clnd <- sector_org_field.by_year.ratio
sector_org_field.by_year.clnd[which (!sector_org_field.by_year.clnd$level_one %in% top6), 2] <-
  "Other"
sector_org_field.by_year.clnd[which (sector_org_field.by_year.clnd$level_one == "United States Government"), 2] <-
  "Other"
...
patents.by_agency <-
  in.patent_level.ratio %>% group_by (level_one) %>% summarize (count = sum(weight))
...
gov.archive <-
  in.sector.archive[which(in.sector.archive$thes_types == "Government"),]
