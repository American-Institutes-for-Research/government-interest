# create visualizations for Andy's DOE talk
# sarora@air.org
# may 2017
...
# test this merge looks legit with patent_ids 4181882, RE45930, 7340129
View(in.patent_level.merged)
in.patent_level.merged[which(in.patent_level.merged$patent_id=='4181882'), ]
in.gov_level[which(in.gov_level$patent_id=='4181882'),] # 1.0 weight 
in.patent_level.merged[which(in.patent_level.merged$patent_id=='RE45930'), ]
in.gov_level[which(in.gov_level$patent_id=='RE45930'),] # 0.0 weight 
in.patent_level.merged[which(in.patent_level.merged$patent_id=='7340129'), ]
in.gov_level[which(in.gov_level$patent_id=='7340129'),] # 0.5 weight

# remove non-DEO patents
doe.patent_ids <- unique(in.patent_level.merged[in.patent_level.merged$level_one == "Department of Energy", 1])
length(doe.patent_ids) # 22,339

in.patent_level.merged.b4doe <- in.patent_level.merged
in.patent_level.b4doe <- in.patent_level
in.gov_level.b4doe <- in.gov_level

in.patent_level.merged <- in.patent_level.merged[which(in.patent_level.merged$patent_id %in% doe.patent_ids), ]
in.patent_level <- in.patent_level[which(in.patent_level$patent_id %in% doe.patent_ids), ]
in.gov_level <- in.gov_level[which(in.gov_level$patent_id %in% doe.patent_ids), ]

length(unique(in.patent_level.merged$patent_id)) # 22,339
length(in.patent_level$patent_id)
length(unique(in.gov_level$patent_id))
...
g.5l <- ggplot(keep.5a, aes(x = year, y = freq, colour=wipo_field, group = wipo_field, linetype=wipo_field)) + 
  geom_line( size=1.5) + 
  ylab(label="Number of DoE Patents") +  xlab("Year") +
...
g.5l
ggsave (paste0("out\\doe\\longWipoFields_v", script_v, ".png"), device = "png")
...
g.5lp <- ggplot(merged.keep5) + 
  ...
  ylab(label="Percent of DoE GI Patents to All Patents") +  xlab("Year") + 
  scale_colour_manual(values=c(cyan, darkGrey, darkGreen, darkRed, darkPurple, lightGrey)) +
...
g.5lp
ggsave (paste0("out\\doe\\longWipoFieldsPercent_v", script_v, ".png"), device = "png")
...
g.12 <- ggplot(melt.ind) + 
  ...
  scale_colour_manual(values=c(cyan, darkGrey, darkGreen), labels=c( "GI DoE patents", "All patents", "Federal R&D funding")) +
  scale_linetype_manual(values=c("solid", "twodash", "dashed"), labels=c( "GI DoE patents", "All patents", "Federal R&D funding")) +
  scale_x_continuous(breaks=c(1980,1985,1990,1995,2000,2005,2010,2015)) +
  theme_set(theme_gray(base_size = 16)) + 
  theme(legend.direction = 'horizontal', 
        legend.position = 'bottom',
...
g.12
ggsave (paste0("out\\doe\\indexed_", script_v, ".png"), device = "png")
...
g.13 <- ggplot(melt.agg) + 
  ...
  scale_colour_manual(values=c(cyan, darkGrey), labels=c("GI DoE patents", "All patents")) +
  scale_linetype_manual(values=c("solid", "twodash"), labels=c("GI DoE patents", "All patents")) +
  theme(legend.direction = 'horizontal', 
        legend.position = 'bottom',
        legend.key = element_blank(),
...
g.13
ggsave (paste0("out\\doe\\longInventor_v", script_v, ".png"), device = "png")
...
# doe script correction
count.ass_org.srtd$source <- 0
...
p.sk <- plot_ly(
  ...
  layout(
    title = "DoE Interest Patents to Assignees, 1980 - 2017",
    font = list(
      size = 10
    ),
...
p.sk
export(p.sk, file = paste0("out\\doe\\Sankey_", script_v, "_", ".png"))
htmlwidgets::saveWidget(p.sk, file = paste0("F:\\Govt_Int\\Final_CSVS\\out\\doe\\Sankey org name_", script_v, "_", ".html"))
...
in.sector[in.sector$patent_id == '4363504', ]  # for testing try 9203030, 7595023, 6824972, 7138569, and 4180935; doe: 4363504
level_one.sub[level_one.sub$patent_id == '4363504', ]
sector_org.merged[sector_org.merged$patent_id == '4363504' , ]
...
sector_org.merged.ratio.clnd[sector_org.merged.ratio.clnd$patent_id == '9203030',] # spot testing
...
g.18b
ggsave (paste0("out\\doe\\funders-assignees.dodged_", script_v, ".png"), device = "png")
...
patent_size.merged.clnd[patent_size.merged.clnd$patent_id == '9203030',] # spot testing
...
patent_size.short[patent_size.short$patent_id == '9203030',] # spot testing
...
g.19n <- ggplot(patent_size.cnt, aes(x = year, y = freq, colour=type, group = type, linetype=type)) + 
  geom_line(size=1.5) + 
  ylab(label="Weighted Number of DoE Interest Patents") +  xlab("Year") + 
  scale_x_continuous(breaks=c(1990,1995,2000,2005,2010,2015)) +
  scale_y_continuous(label=comma, breaks=c(0,100,200,300,400,500, 600, 700)) +
...
g.19n
ggsave (paste0("out\\doe\\firmSize_", script_v, ".png"), device = "png")
...
in.patent_level[in.patent_level$patent_id=='4363504', ] # 4181882, a 1980 patent; also test 6755975, a 2004 patent; 9203030, a 2015 patent

# quick test
cit.merge_5[cit.merge_5$patent_id == '4363504', ]
sector_org.merged.ratio.clnd[sector_org.merged.ratio.clnd$patent_id == '4363504' , ] 
...
head(sector_merge)
View(sector_merge[is.na(sector_merge$thes_types), ])

sector_merge.sub <- sector_merge[,c(23:29)]
sector_merge.sub[is.na(sector_merge.sub)] <- 0
sector_merge[sector_merge$patent_id=='4181882',]
sector_merge.sub[which(sector_merge$patent_id=='4181882'),]
...
cit_sector.count
cit_sector.count.clnd <- cit_sector.count[cit_sector.count$thes_types != 0,]
...
g.27 <- ggplot(cit_sector.count.melt.clnd, aes(x=variable,y=value,fill=thes_types)) +
  geom_bar(stat="identity", position = "dodge") + 
  labs(y= "Average Accrued Weighted Citations for DoE GI Patents", x="Year After Publication") +
  scale_y_continuous(label=comma) +
...
g.27
ggsave (paste0("out\\doe\\fiveYearCitationImpact_", script_v, ".png"), device = "png")
...
write.csv (sector_org_field.by_year.count, file="out\\doe\\sector_org_field.by_year.count.csv")
write.csv (freq.all.by_year, file="out\\doe\\sector_org_field.by_year.count")
write.csv (freq.all.by_year, file="out\\doe\\freq.all.by_year.csv")
write.csv (freq.gi.by_year, file="out\\doe\\freq.gi.by_year.csv")
write.csv (in.patent_level.merged, file="out\\doe\\out.patent_level.merged.csv")
write.csv (patents.keep_ids, file="out\\doe\\patents.keep_ids.csv")
write.csv (merged.keep5, file = "out\\doe\\merged_wipo_fields.keep6.csv")
write.csv (long.5a, file = "out\\doe\\gi.wipo_fields.keep6.csv")
write.csv (patent_size.cnt, file = "out\\doe\\patent_size.cnt.csv")
write.csv (cit_sector.count.melt.clnd, file = "out\\doe\\citations_accrued_years_1_thru_5.csv")
write.csv (in.patent_level.ratio, file = "out\\doe\\weighted_gi_patents.by_funding_agency.csv")
write.csv (cit_sector.count.melt.clnd, file = "out\\doe\\cit_sector.count.melt.clnd.csv")
