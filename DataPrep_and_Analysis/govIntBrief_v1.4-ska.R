...
in.fund <- read.csv("FFS2015_DST_001.csv", header = TRUE, stringsAsFactors = FALSE)
in.fund.bkp <- in.fund
in.fund[1:10,]
colnames(in.fund)
in.fund$Total.R.D <- as.numeric(in.fund$Total.R.D)
in.fund$Fiscal.year <- as.numeric(in.fund$Fiscal.year)
in.fund$Federal.Adusted <- as.numeric(in.fund$Federal.Adusted)
in.fund$FederalIndex <- in.fund$Federal.Adusted / in.fund[in.fund$Fiscal.year==1980, which(colnames(in.fund)=="Federal.Adusted")] * 100
...
melt.ind <- melt(merged.ind2, id="year", measure.vars = c(3,5,13))
colnames(melt.ind)
View(melt.ind )
...
g.12 <- ggplot(melt.ind) + 
  ...
  scale_colour_manual(values=c(darkGreen, cyan, darkGrey, darkRed), labels=c( "Government interest patents", "All patents", "R&D funding")) +
  scale_linetype_manual(values=c("dashed", "solid", "twodash"), labels=c( "Government interest patents", "All patents", "R&D funding")) +
...
head(count.new_entrants.small)

# TODO: Have RA QA new entrants once 2016 data come thru.  -- ska 1/31
