...
in.patent_level[in.patent_level$patent_id=='6755975', ] # 4181882, a 1980 patent; also test 6755975, a 2004 patent; 9203030, a 2015 patent

# quick test
cit.merge_5[cit.merge_5$patent_id == '6755975', ]
sector_org.merged.ratio.clnd[sector_org.merged.ratio.clnd$patent_id == '6755975' , ] 
...
which (is.na(sector_merge.sub))
sector_merge.sub[is.na(sector_merge.sub)] <- 0
which (is.na(sector_merge.sub))
sector_merge[sector_merge$patent_id=='6755975',]
sector_merge.sub[which(sector_merge$patent_id=='6755975'),]

colnames(sector_merge)
# cit_sector.count <- aggregate(cbind (weight, First, Second, Third, Fourth, Fifth) ~
            # thes_types, sum, na.rm= TRUE, data = sector_merge.sub[which(sector_merge$patent_id=='6755975'),]) # testing only
cit_sector.count <- aggregate(cbind (weight, First, Second, Third, Fourth, Fifth) ~
