...
head(in.gender_wipo)
nrow(in.gender_wipo)
in.gender_wipo[which(in.gender_wipo$patent_id == '5408874'), ]
...
women_in_teams[which(women_in_teams$patent_id == '5408874'), ]
View(women_in_teams)

grouped_by.num_inventors <- women_in_teams %>% group_by(num_inventors) %>% summarise(meanPercWomenOnTeam = mean(percentWomen), numPatents = length(patent_id))
...
graph3 <- ggplot(data=for_graph.num_inventors, aes(x=num_inventors, y=meanPercWomenOnTeam)) +
  ...
  labs(y= "Mean Percentage of Women on Teams", x="Inventor Team Size", colour="Number of\nPatents", size="Number of\nPatents") +
  theme_set(theme_gray(base_size = 16)) + 
...
for_graph_inventors_team$has_fem_inv <- as.character(for_graph_inventors_team$has_fem_inv)

graph4 <- ggplot(data=for_graph_inventors_team, aes(x=num_inventors_clean, y=PercentageofInventors, fill=has_fem_inv)) +
  geom_point() + labs(y= "Percentage of Patents", x="Number of Inventors Associated with Patent", fill = "With at least one female inventor") +
