...
for_graph_types[which(for_graph_types$thes_types == 'Ambiguous' | for_graph_types$thes_types == 'Hospital' | for_graph_types$thes_types == 'Person'), 2] <- "Other"
...
graph2 <- ggplot(for_graph_types, aes(x=year,y=PercentageofAssignees,group=thes_types,fill=thes_types)) +
  geom_area(position="fill") + labs(y= "Percentage of Assignees", x="Year", fill = "Type of Assignee") +
  scale_fill_manual(values=c(darkPurple, darkBlue, darkGreen, lightGrey))
graph2
