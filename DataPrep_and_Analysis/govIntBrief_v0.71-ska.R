...
g.13 <- ggplot(melt.agg) + 
  ...
  scale_colour_manual(values=c("red","orange"), labels=c("Mean government\ninterest inventors", "Mean inventors\nacross all patents")) +
  theme(legend.direction = 'horizontal', 
        legend.position = 'bottom',
        legend.key = element_rect(size = 5),
        legend.key.size = unit(1.5, 'lines'),
...
g.13
ggsave (paste0("out\\inventors\\longInventorWipoFields_v", script_v, "_", i, ".png"), device = "png")

# 14a. mean number of inventors for gi patents by top 10 wipo_fields
...
g.14a <- ggplot(inv_wipo.by_year, aes(x = Year, y = x, colour=Wipo_Field, group = Wipo_Field)) + 
  geom_line(size=1.2) + 
...
g.14a
ggsave (paste0("out\\longInventorWipoFields_v", script_v, "_", i, ".png"), device = "png")

#14.b drill down into inventor trends 
gi_patents.merged <- in.patent_level.merged[which(in.patent_level.merged$avg_gi > 0),]
gi_patents.merged$wipo_field <- as.character(gi_patents.merged$wipo_field)
gi_inventor.dist <- count (gi_patents.merged$num_inventors) 
gi_inventor.dist$percent <- gi_inventor.dist$freq / nrow(gi_patents.merged)

all_inventor.dist <- count (in.all$num_inventors)
all_inventor.dist$percent <- all_inventor.dist$freq / nrow (in.all)

inventor.cats <- 5
wipo.fields <- 5

results <- data.frame(matrix(ncol = 5, nrow = inventor.cats * wipo.fields))
colnames(results) <- c("Number of inventors", "Percent of GI patents in inventor category", "Percent of ALL patents in inventor category", "Top WIPO fields in GI patents", "Top WIPO fields in ALL patents")
results$`Number of inventors` <- rep(1:inventor.cats, each= wipo.fields)

inventor_fields.results = list ()
patent_percent.results = list ()
  
for (j in 1:inventor.cats) {
  if (j < inventor.cats) {
    subset.inventors <- gi_patents.merged[gi_patents.merged$num_inventors == j,]
  }
  else {
    subset.inventors <- gi_patents.merged[gi_patents.merged$num_inventors >= j,]
  }
  
  topWipoFields <- count(subset.inventors$wipo_field)
  topWipoFields$x <- as.character(topWipoFields$x)
  topWipoFields <- topWipoFields[order(-topWipoFields$freq),]
  inventor_fields.results[[j]] <- topWipoFields[1:wipo.fields, 1]

  patent_percent.results[[j]] <- rep(nrow(subset.inventors) / nrow(gi_patents.merged), each = wipo.fields)
}

results$`Percent of GI patents in inventor category` <- unlist(patent_percent.results)
results$`Top WIPO fields in GI patents` <- unlist(inventor_fields.results)
...
for (j in 1:5) {
  print (j)
  if (j < 5) {
    subset.inventors <- in.all[in.all$num_inventors == j,]
  }
  else {
    subset.inventors <- in.all[in.all$num_inventors >= j,]
  }
  ...
  patent_percent.results[[j]] <- rep(nrow(subset.inventors) / nrow(in.all), each = wipo.fields)
}

results$`Percent of ALL patents in inventor category` <- unlist(patent_percent.results)
results$`Top WIPO fields in ALL patents` <- unlist(inventor_fields.results)

write.csv (results, file="out\\inventor\\inventor.results.csv")

View(results)

# 14c. mean number of inventors for gi patents by select wipo fields
wipo.fields <- unique(c(results$`Top WIPO fields in ALL patents`, results$`Top WIPO fields in GI patents`))
...
head (in.patent_level.merged)
filtered <- in.patent_level.merged[which (in.patent_level.merged$year >= 1980 & in.patent_level.merged$year <= 2015), ]
in.patent_level.merged.top_wipo <- filtered[which(filtered$wipo_field %in% wipo.fields & filtered$avg_gi == 1), c(5,6,2)]
head(in.patent_level.merged.top_wipo)

inv_wipo.by_year <- aggregate (in.patent_level.merged.top_wipo[,3], by = list (Wipo_Field = in.patent_level.merged.top_wipo$wipo_field, Year = in.patent_level.merged.top_wipo$year), na.rm = TRUE, mean)
head (inv_wipo.by_year)

g.14c <- ggplot(inv_wipo.by_year, aes(x = Year, y = x, colour=Wipo_Field, group = Wipo_Field)) + 
  geom_line(size=1.2) + 
  ylab(label="Mean number of inventors") +  xlab("Year") + scale_x_continuous(breaks=c(1980,1990,2000,2010)) +
  theme(legend.direction = 'horizontal', 
        legend.position = 'bottom',
...
g.14c
ggsave (paste0("out\\inventor\\longInventorWipoFields_GI.patents_v", script_v, "_", i, ".png"), device = "png")


# 14d. mean number of inventors for gi patents by select wipo fields
colnames(in.all)
in.all$wipo_field <- as.character(in.all$wipo_field)
in.all$year <- as.numeric(as.character(in.all$year))
head (in.all)
filtered <- in.all[which (in.all$year >= 1980 & in.all$year <= 2015), ]
in.all.top_wipo <- filtered[which(filtered$wipo_field %in% wipo.fields), c(5,6,2)]
head(in.all.top_wipo)

inv_wipo.by_year <- aggregate (in.all.top_wipo[,3], by = list (Wipo_Field = in.all.top_wipo$wipo_field, Year = in.all.top_wipo$year), na.rm = TRUE, mean)
head (inv_wipo.by_year)

g.14d <- ggplot(inv_wipo.by_year, aes(x = Year, y = x, colour=Wipo_Field, group = Wipo_Field)) + 
  geom_line(size=1.2) + 
  ylab(label="Mean number of inventors") +  xlab("Year") + scale_x_continuous(breaks=c(1980,1990,2000,2010)) +
  theme(legend.direction = 'horizontal', 
        legend.position = 'bottom',
...
g.14d
ggsave (paste0("out\\inventor\\longInventorWipoFields_ALL.patentsv", script_v, "_", i, ".png"), device = "png")
...
in.boundary <- read.csv("GI_connection_to_private_industry.csv", header = TRUE, stringsAsFactors = FALSE)

colnames(in.all)

tmp <- table(in.all$num_times_cited_by_us_patents)
tmp.df <- as.data.frame(tmp)
tmp.df <- as.data.frame(tmp)
tmp.df$Var1 <- as.numeric(as.character(tmp.df$Var1))
colnames(tmp.df)

fit1 <- fitdistr(log(in.all$num_times_cited_by_us_patents + 1), "exponential")
fit2 <- fitdistr(log())

g21 <- ggplot(in.all, aes(x = log(num_times_cited_by_us_patents + 1))) +
  stat_function(fun = dexp, geom = "area", fill = "blue", alpha = .25, 
