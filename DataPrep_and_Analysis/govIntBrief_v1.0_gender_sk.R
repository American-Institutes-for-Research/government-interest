...
ggsave (paste0("out\\gender_v2\\gi_teams_with_female_inventors_over_time.png"), device = "png")


#################################################
#make the counts of teams with at least one woman by year and team size

#make a categorical variable for team size
...
ggsave (paste0("out\\gender_v2\\line_teams_with_fem_inv_over_time_by_teamsize.png"), device = "png")
...
graph2 <- ggplot(data=for_graph_inventors_team, aes(x=num_inventors_clean, y=PercentageofInventors, fill=has_fem_inv)) +
  ...
  scale_fill_manual(values=c(cyan, darkPurple, darkGreen),labels=c( "All Male Inventors", "Atleast one Woman", "Unknown")) + 
  theme_set(theme_gray(base_size = 16)) + 
  theme(legend.direction = 'horizontal', 
        legend.position = 'bottom',
        legend.key = element_blank(),
...
ggsave (paste0("out\\gender_v2\\percent_with_female_inventor_by_team_size.png"), device = "png")